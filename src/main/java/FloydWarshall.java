...
public class FloydWarshall extends Algorithm {
	...
	private static final String GRAPH_ADJ_FILE = "./graph-floyd-warshall-example.adj";

	private Double [][] ADJ;
	
	public FloydWarshall() {
		try {	
			BufferedReader reader = new BufferedReader(new FileReader(new File(GRAPH_ADJ_FILE)));
			...
			for (int i=0; i<n; i++) {
				...
				for (int j=0; j<n; j++) {
					ADJ[i][j] = Double.parseDouble(line[j]);
				}
			}
		}
		catch (Exception e) {
